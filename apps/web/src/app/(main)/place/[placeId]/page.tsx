      ...
      <div className="px-4 pt-4 pb-3 space-y-2">
        ...
        {place.formattedAddress && (
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <MapPin size={11} /><span>{place.formattedAddress}</span>
          </div>
        )}
        ...
        <div className="flex gap-3 pt-1">
          ...
          <Button variant="outline" className="flex-1 font-heading font-semibold border-border text-foreground h-12 rounded-xl" asChild>
            <a href={mapsUrl} target="_blank" rel="noopener noreferrer">
              <Navigation size={16} className="mr-1.5" />Itinéraire
            </a>
          </Button>
        </div>
      </div>
          ...
          <div className="px-4 space-y-6">
            {place.rating != null && (
              <div className="bg-secondary rounded-2xl p-4 flex items-center gap-4">
                <div className="flex flex-col items-center">
                  <span className="font-heading font-extrabold text-4xl text-foreground">{place.rating.toFixed(1)}</span>
                  <div className="flex items-center gap-0.5 mt-1">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Star key={s} size={12} className={s <= Math.round(place.rating!) ? 'fill-accent text-accent' : 'text-border'} />
                    ))}
                  </div>
                </div>
                <div className="flex-1 border-l border-border pl-4">
                  <p className="font-heading font-bold text-foreground">{place.userRatingCount?.toLocaleString('fr-FR')} avis</p>
                  <p className="text-xs text-muted-foreground mt-0.5">sur Google Maps</p>
                  {place.websiteUri && (
                    <a href={place.websiteUri} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-primary mt-2">
                      <Globe size={11} />Site web<ExternalLink size={10} />
                    </a>
                  )}
                  {place.internationalPhoneNumber && (
                    <a href={`tel:${place.internationalPhoneNumber}`} className="flex items-center gap-1 text-xs text-primary mt-1">
                      <Phone size={11} />{place.internationalPhoneNumber}
                    </a>
                  )}
                </div>
              </div>
            ...
            {aiSummary && (
              <div className="space-y-2">
                ...
                <p className="text-sm text-foreground leading-relaxed bg-secondary rounded-2xl p-4">{aiSummary}</p>
              </div>
            )}
                  ...
                  <div key={review.name} className="bg-secondary rounded-2xl p-4 space-y-2">
                    <div className="flex items-center gap-2">
                      {review.authorAttribution?.photoUri ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={review.authorAttribution.photoUri} alt={review.authorAttribution.displayName} className="w-8 h-8 rounded-full object-cover" />
                      ) : (
                        <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center">
                          <span className="text-sm font-bold text-muted-foreground">{review.authorAttribution?.displayName?.[0] ?? '?'}</span>
                        </div>
                      )}
                      <div>
                        <p className="font-heading font-semibold text-sm text-foreground">{review.authorAttribution?.displayName}</p>
                        <div className="flex items-center gap-1">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star key={s} size={10} className={s <= review.rating ? 'fill-accent text-accent' : 'text-border'} />
                          ))}
                          <span className="text-xs text-muted-foreground ml-1">{review.relativePublishTimeDescription}</span>
                        </div>
                      </div>
                    </div>
                    {review.text?.text && (
                      <p className="text-sm text-foreground leading-relaxed line-clamp-4">{review.text.text}</p>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-1">
                  {(place.photos ?? []).slice(1, 10).map((photo, i) => (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img key={i} src={photo.url} alt={`${place.displayName.text} photo ${i + 2}`}
                      className="aspect-square w-full object-cover rounded-lg" loading="lazy" />
                  ))}
                </div>
              </div>
            )}
